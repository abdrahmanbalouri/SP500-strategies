...
def sp500_cum(dates):
    close = (
        pd.read_csv(os.path.join(ROOT, "data", "HistoricalData.csv"), parse_dates=["Date"])
        .set_index("Date")
        .sort_index()["Close"]
    )
...
if __name__ == "__main__":
    ...
    folds = joblib.load(os.path.join(MODEL_DIR, "folds.pkl"))
...
## Cross-validation
Blocking Time Series Split, 10 folds (`BlockingTimeSeriesSplit` + `cross_validate` / `cross_val_predict`).
